...
public class JWTAuthorizationFilter extends BasicAuthenticationFilter {
    ...
    private UsernamePasswordAuthenticationToken getAuthentication(HttpServletRequest request) {
        String token = request.getHeader(Config.JWT_HEADER_STRING);
        if (token != null) {
            String user = JWT.require(Algorithm.HMAC512(Config.JWT_SECRET.getBytes())).build()
                    .verify(token.replace(Config.JWT_TOKEN_PREFIX, "")).getSubject();
